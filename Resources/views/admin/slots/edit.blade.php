...
@section('content-header')
    <h1>
        {{ trans('ibooking::slots.title.edit slot') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.ibooking.slot.index') }}">{{ trans('ibooking::slots.title.slots') }}</a></li>
        <li class="active">{{ trans('ibooking::slots.title.edit slot') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.ibooking.slot.update', $slot->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('ibooking::admin.slots.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
