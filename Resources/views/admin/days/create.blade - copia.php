...
@section('content-header')
    <h1>
        {{ trans('ibooking::days.title.create day') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.ibooking.day.index') }}">{{ trans('ibooking::days.title.days') }}</a></li>
        <li class="active">{{ trans('ibooking::days.title.create day') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.ibooking.day.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('ibooking::admin.days.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
